...
const ARLedgerPage = () => {
  ...
  const [arAccountId, setArAccountId] = useState<number | null>(null);
    ...
    if (!authLoading && isAuthenticated && token) {
      if (isAdmin) {
        branchService.getBranches(token)
          .then(data => { setBranches(data); if (data.length > 0) setSelectedBranchId(data[0].id);})
  ...
  const fetchAccountSettingsForBranch = useCallback(async (branchId: number) => {
    ...
    try {
      ...
      const accId = getSingleAccountIdFromSettings(settings, "default_accounts_receivable_account_id");
      if (!accId) {
        setError(`Default Accounts Receivable Account not configured for branch ${branchId}.`);
        setArAccountId(null);
      } else {
        setArAccountId(accId);
      }
    } catch (err: any) {
      setError(err.message || 'Failed to fetch account settings.');
      setArAccountId(null);
    } finally {
      setIsLoading(false); // This loading is for settings, ledger fetch has its own
    }
  }, [token]);
  ...
  const handleViewLedger = async () => {
    if (!token || !selectedBranchId || !arAccountId || !filterStartDate || !filterEndDate) {
      setError("Please select branch, ensure A/R account is set, and select a valid date range.");
      setJournalEntries([]);
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch A/R ledger.');
      setJournalEntries([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Typography variant="h4" component="h1" gutterBottom>Accounts Receivable Ledger</Typography>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} sm={isAdmin ? 3 : 4}><Button variant="contained" startIcon={<FilterListIcon />} onClick={handleViewLedger} fullWidth disabled={isLoading || !selectedBranchId || !arAccountId}>View Ledger</Button></Grid>
        </Grid>
        {!arAccountId && selectedBranchId && !isLoading && <Alert severity="warning" sx={{mt:1}}>A/R Account not set for this branch in Accounting Settings.</Alert>}
      </Paper>

      {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
      {isLoading && <Box display="flex" justifyContent="center" my={3}><CircularProgress /></Box>}

      {!isLoading && dataFetched && arAccountId && (
            ...
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>A/R Account Activity</TableCell> {/* This will be the A/R account name */}
                  <TableCell align="right">Debit</TableCell><TableCell align="right">Credit</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {journalEntries.length === 0 && <TableRow><TableCell colSpan={6} align="center">No A/R entries found for this period.</TableCell></TableRow>}
                {journalEntries.map((entry) =>
                  entry.items.filter(item => item.account_id === arAccountId) // Show item related to A/R
                  .map(item => (

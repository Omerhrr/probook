...
session_start();
	include('../connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM supliers WHERE branch = '{$_SESSION['BRANCH']}'AND suplier_id= :userid");
...
<span>Email Address : </span><input type="text" style="width:265px; height:30px;" name="cperson" value="<?php echo $row['email']; ?>" /><br>
<span>Contact No.: </span><input type="text" style="width:265px; height:30px;" name="contact" value="<?php echo $row['contact']; ?>" /><br>

...
session_start();
	include('../connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM deposit WHERE branch = '{$_SESSION['BRANCH']}'AND deposit_id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
<link href="../style.css" media="screen" rel="stylesheet" type="text/css" />
<form action="saveeditdeposit.php" method="post">
<center><h4><i class="icon-edit icon-large"></i> Edit Deposit</h4></center><hr>
<div id="ac">

...
<select name="product" style="width:200px; "class="chzn-select" required>
<option></option>
	<?php
	include('../connect.php');
...
<input type="text" name="date" style="width: 60x; height:30px; padding-top:6px; padding-bottom: 4px; margin-right: 4px; font-size:15px;" value="<?php echo date("Y-m-d"); ?>" />
...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include('../connect.php');
				$result = $db->prepare("SELECT * FROM sales_order WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= :userid ");
			...
			<tr>
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->prepare("SELECT sum(amount) FROM sales_order WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= :a");
			...
			<?php 
				$resulta = $db->prepare("SELECT sum(profit) FROM sales_order WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= :b");
...
<a rel="facebox" href="checkout.php?pt=<?php echo $_GET['id']?>&invoice=<?php echo $_GET['invoice']?>&total=<?php echo $fgfg ?>&totalprof=<?php echo $asd ?>&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>&dates=<?php echo $_GET['date']?>"><button class="btn btn-success btn-large btn-block"><i class="icon icon-save icon-large"></i> SAVE</button></a>

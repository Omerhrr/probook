...
<body>
	...
	<div class="span10">
	<a href="customer_ledger.php?cname="><button class="btn btn-default"><i class="icon-arrow-left"></i> Back to Customer Ledger</button></a>
...
<div style="margin: 0 auto; padding: 20px; width: 900px; font-weight: normal;">
 ...
 ?>
	...
	$resulta = $db->prepare("SELECT * FROM c_ledger WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= '{$a}'");
	...
	$resulta = $db->prepare("SELECT *,sum(debit_balance) as udebit FROM c_ledger WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= '{$a}'");
	//$resulta->bindParam(':a', $id);
	$resulta->execute();
	for($i=0; $rowa = $resulta->fetch(); $i++){
	$address=$rowa['transaction_id'];
	...
	$resulta = $db->prepare("SELECT *,sum(debit) as debits, sum(credit) as credits FROM c_ledger WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= '{$a}'");
	
	$result = $db->prepare("SELECT *,sum(debit) as debits, sum(credit) as credits FROM c_ledger WHERE branch = '{$_SESSION['BRANCH']}' AND name= :userid ORDER BY transaction_id ASC");
	...
	?>
				...
				<tr>
					<td colspan="5" style=" text-align:right;"><strong style="font-size: 12px;">Balance: &nbsp;</strong></td>
					<td colspan="2"><strong style="font-size: 12px;">
					<?php
					$d_balance=$rowa['debit_balance'];
				...
				<tr>
					...
					<?php
					$d_balance=$row['debits']- $row['credits'];
					echo formatMoney($d_balance, true);
					?>
					</strong></td>
					<?php 

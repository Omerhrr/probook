    ...
    <link rel="stylesheet" type="text/css" href="css/side.css">
     ...
     echo formatmoney($tot_sale,true);
      ...
      ?>
        ...
        $result = $db->prepare("SELECT sum(cost) as cost From purchases_item WHERE description = 'Expenses'  and branch = '{$_SESSION['BRANCH']}'");
    ...
    echo formatMoney($expense,true); 
      ...
      ?>
       ...
       $result = $db->prepare("SELECT sum(cost) as cost  FROM sales_order WHERE branch = '{$_SESSION['BRANCH']}' ORDER BY product_code");
        ...
        $incomes = $db->prepare("SELECT SUM(amount) as amount FROM incomelist WHERE branch = '{$_SESSION['BRANCH']}' ORDER BY invoice");
       $incomes->execute();

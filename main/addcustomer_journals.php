...
session_start();
	include('../connect.php');
	$id=$_GET['iv'];
	$result = $db->prepare("SELECT * FROM c_ledger WHERE branch = '{$_SESSION['BRANCH']}' AND invoice= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
<link href="../style.css" media="screen" rel="stylesheet" type="text/css" />
<form action="savecustomer_journals.php" method="post">
...
<span>Customer:</span>
<select name="customer" style="width:265px; height:30px; margin-left:-5px;" >
	<option></option>
	<?php
	$results = $db->prepare("SELECT * FROM customer WHERE branch = '{$_SESSION['BRANCH']}'");
...
<input type	="hidden" style="width:265px; height:30px;" name="d_bal" value="<?php echo $row['debit_balance']; ?>" /><br>

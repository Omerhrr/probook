...
<form action="admin_general_ledger.php?branch=" method="get">
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: center;">
	...
	<tbody>
			...
			$branch=$_GET['branch'];
				...
				$result = $db->prepare("SELECT *,debit, credit FROM c_ledger WHERE branch = '{$branch}' ORDER BY transaction_id DESC ");
			...
			<td style="text-align: right"><?php echo formatMoney($row['debit_balance'],true); ?></td>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: center;">
	...
	<tbody>
			...
			$branch=$_GET['branch'];
				...
				$result = $db->prepare("SELECT *, debit, credit FROM s_ledger WHERE branch = '{$branch}'  ORDER BY transaction_id DESC ");
			...
			<td style="text-align: right"><?php echo formatMoney($row['debit'],true); ?></td>
			<td style="text-align: right"><?php echo formatMoney($row['credit'],true); ?></td>
			<td style="text-align: right"><?php echo formatMoney($row['credit_balance'],true); ?></td>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: center;">
	...
	<tbody>
			...
			$branch=$_GET['branch'];
				...
				$result = $db->prepare("SELECT * FROM sales_order WHERE branch = '{$branch}'ORDER BY transaction_id DESC");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
					$invoice = $row['invoice']
			?>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: center;">
	...
	<tbody>
			...
			<?php
				$branch=$_GET['branch'];
				$result = $db->prepare("SELECT * FROM purchases JOIN purchases_item on purchases_item.invoice = purchases.invoice_number  WHERE purchases.branch = '{$branch}' AND purchases_item.description ='purchases' ORDER by purchases.date DESC ");
			...
			<td style="text-align: right"><?php
			$rrrrrrr=$row['name'];
			$resultss = $db->prepare("SELECT * FROM products WHERE branch = '{$branch}' AND product_code= :asas");
			$resultss->bindParam(':asas', $rrrrrrr);
			$resultss->execute();
			for($i=0; $rowss = $resultss->fetch(); $i++){
			echo formatMoney($rowss['o_price'],true);
			}
			?></td>
